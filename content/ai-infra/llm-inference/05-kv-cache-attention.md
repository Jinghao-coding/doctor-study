...
这三者解决不同问题，可以叠加用。KV Cache 解决"避免重复计算历史 K/V"：自回归生成每步都要关注历史上下文，缓存历史 K/V 后每步只算新增 token；它的大小 = 2 × num_layers × num_kv_heads × head_dim × seq_len × dtype_bytes，随序列长度、batch 和 KV head 数线性增长，是长上下文和高并发的核心显存压力。从模型结构上可以用 GQA、MQA、MLA 减少 KV head 数来压它。PagedAttention 不是 attention kernel，而是 KV Cache 的虚拟内存系统：借鉴 OS 分页，把 KV 切成固定 block、用 block table 映射逻辑到物理，按需分配、引用计数加 copy-on-write 共享前缀，把碎片和动态接纳问题解决掉，从而支撑 iteration-level scheduling 和 continuous batching。FlashAttention 则是 attention kernel 的计算访存优化，靠分块和 online softmax 让中间 attention score 矩阵不落 HBM，降低 HBM IO，主要作用在 prefill 和长上下文。面试口径：KV Cache 缓存历史、PagedAttention 管显存、FlashAttention 减 HBM IO。
