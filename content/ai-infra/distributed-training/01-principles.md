...
分布式训练并行策略的总公式是总 GPU 数由 DP、TP、PP 等维度相乘决定。 先画并行维度，再讲通信和显存代价。

**2 分钟版：**

分布式训练不是卡越多越快，而是在多张 GPU 之间拆数据、拆模型、拆训练状态、放通信这四个问题。数据拆用 DP/DDP，每卡处理不同 batch，每步 AllReduce 同步梯度；模型拆里 TP 切层内矩阵、PP 切层间 stage、EP 切 MoE 专家；状态拆用 ZeRO/FSDP，把参数、梯度、优化器状态从每卡完整保存改成分片。选型按"先能放下、再跑得快、再排得好"：单卡放不下先上 ZeRO/FSDP，单层太大上 TP，层数太多上 PP，吞吐不够再扩 DP。常见组合是 3D 并行，总 GPU 数等于 TP×PP×DP。放置原则比公式更关键：TP 通信最频繁要限制在节点内 NVLink/NVSwitch，PP 只传相邻 stage 激活可跨节点，DP 每步同步一次梯度放最外层扩吞吐。落地时通信代价和显存代价是核心权衡，要结合拓扑判断瓶颈在哪一维。
