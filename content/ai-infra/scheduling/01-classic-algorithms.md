...
经典调度算法是所有调度系统的排序基础，每个都有明确的最优和失败条件：FIFO 实现最简单但有 head-of-line blocking，SJF 最小化平均等待时间但会饿死长任务，SRTF 是可抢占版适合不断到达的短任务，EDF 实时最优但过载会集体崩溃，Round Robin 公平但 GPU 任务切换代价高几乎不用，优先级最常用但低优要靠 aging 防饥饿。真实调度器是组合使用：排序用优先级+SJF，放置层用 Bin Packing 减碎片或 Spread 保高可用。

**2 分钟版：**

经典调度算法都来自操作系统，但底层排序思想适用于任何调度器。我会先按"最优性 vs 失败条件"过一遍：FIFO 按到达顺序，简单、不需要预测、对同质任务够用，但队首一个长任务会 head-of-line blocking 阻塞后面所有短任务；SJF 短任务优先，能证明它最小化平均等待时间，但短任务源源不断时长任务会饥饿，需要 aging 或配额保障；SRTF 是 SJF 的可抢占版，按剩余时间排序，适合任务不断到达的场景，代价是需要抢占机制和更准的剩余时间预测；EDF 按 deadline 排序，在可抢占且 deadline 可知时是实时最优，但过载会集体崩溃，GPU 训练通常没明确 deadline 所以用得少；Round Robin 绝对公平但上下文切换开销大，GPU 任务切换要重建 NCCL，所以几乎不用，只有队列级轮转借鉴了它；优先级最常用，分静态和动态（aging、QAD）。然后我会强调真实系统不是单算法，而是分阶段组合：排序用优先级+SJF，准入做 Gang+Quota 检查，放置用 Bin Packing 或 Spread 加拓扑打分，抢占用代价感知。落到 AI Infra，放置层最关键的是 Bin Packing（减碎片、留完整节点给大 gang）和 Spread（在线推理保高可用、降故障爆炸半径）的取舍。
