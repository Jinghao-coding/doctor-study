...
DeepShare 解决多租户 GPU 集群里的核心矛盾：静态 quota 利用率低，过度共享又破坏 QoS。它提出统一指标 QAD（配额保障度），衡量租户当前 Guaranteed 需求被满足的程度，并用 QAD 同时驱动弹性配额借用、预测性调度和干扰感知合用。在 K8s 集群上把利用率从 39.64% 提到 70.58%，排队延迟降 46%，QoS 合规率 93%。

**2 分钟版：**

DeepShare 是一篇多租户 GPU 集群资源管理的论文。问题背景是：严格按 quota 静态隔离会让闲置资源浪费、利用率只有约 40%，但允许借用后又难保证原主需要时能及时收回。论文认为根因是 quota、调度、抢占、共享四个模块缺少统一控制信号，于是提出 QAD = 已满足 Guaranteed 资源 / min(quota, 当前 Guaranteed demand)，并做 EMA 平滑。QAD 同时驱动三个子系统：DRA 弹性配额借用，空闲资源借出、QAD 下降时按优先级回收；预测性调度，用 Random Forest 预测运行时间，按 QAD 升序加短作业优先的词典序排队，并做代价感知抢占；干扰感知合用，用硬件计数器预测共享保持率（R² 0.902），只在高于动态阈值时合用并运行时驱逐。整体实现为 Kubernetes scheduler plugin，端到端调度延迟小于 50ms。面试里我会强调它的卖点不是单点算法，而是用 QAD 把资源效率和 QoS 保障形式化并闭环。
