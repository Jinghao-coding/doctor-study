...
Maestro 解决 LLM 多智能体工作流的推理调度。它不把请求当黑盒，而是用 agent 上下文做两阶段输出长度预测，再用 CUDA VMM 虚拟内存超配做弹性 KV 缓存，配合工作流感知的 SRTF 调度。64×A100、14.4 万 stage 上，SLO 达成率比 EDF 高 23.6pp，KV 预留内存降 67.2%，交互排队延迟降 84.8%。

**2 分钟版：**

Maestro 面向的是多个 agent 组成 DAG 协作工作流的 LLM 推理场景，一个用户请求会触发几十次 LLM 调用（stage），带来输出长度双峰波动、多模型共占显存、交互与批处理混合延迟三大挑战。核心思路是利用 agent 角色、工作流位置等上下文做前瞻预测，再指导内存和调度。预测分两阶段：LightGBM 先判断是否触发工具调用（AUC 0.96），回归器在给定模式下预测 token 数（MAE 165，R² 0.78）。内存上用五状态模型管理加 CUDA VMM 三倍超配，靠虚拟物理页分离、准入控制和映射失败拒绝三层防护避免 OOM。调度用基于剩余时间的 SRTF 加 stage 边界抢占。关键权衡是预测偏向高估——低估会导致解码中途 OOM 和队头阻塞，危害远大于高估。面试里我会用它说明如何用轻量预测为调度器提供前瞻信号，并讲清工程上不做 token 级抢占的复杂度取舍。
