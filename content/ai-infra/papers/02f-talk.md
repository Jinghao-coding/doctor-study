...
面试讲 DeepShare，不要一上来堆 QAD 公式和 Kubernetes 扩展点。最清楚的顺序是：**固定配额与资源共享的矛盾 → 现有方案缺少统一保障信号 → 用 QAD 统一三个资源管理动作 → 两个预测模块分别提供“时间信息”和“空间干扰信息” → 用 Kubernetes Scheduler Plugin 落地并用实验闭环。**
...
- `QAD 记忆模型`：逐项解释分子、分母、边界和 EMA。
- `总体架构`：Controller 与 Scheduler Plugin 的职责边界。
- `K8S 实现细节`：两级队列、五个扩展点与抢占路径。
- `论文延伸问答`：参数、消融、敏感性、相关工作与局限。
