...
这一节是 DeepShare 的论文级深度问答，全部对应原稿参数和实验。比如 EMA λ=0.3 加 50ms 周期能在约 350ms 内累积到稳态 90%、过滤亚秒抖动；干扰预测用 RF 是因为 sub-ms 推理且 R² 0.902，主特征是 SM Active；平均 JCT 只比 Lucid 好 6.3% 但排队降 46%、tail JCT 降 23%，因为 DeepShare 主战场是排队公平和尾延迟。

**2 分钟版：**

这一节准备的是面试官按论文细节深挖时的应答。EMA λ=0.3、50ms 周期的物理意义是持续偏差约 350ms 累积到稳态 90%，既过滤短任务抖动又能快速发现持续 under-service。QAD 在 demand=0 时定为 1 而非 0，避免空闲租户被误判为最受损者；分母用 min(quota, demand) 防止虚报哄抬。RF 选型三点理由：sub-ms 推理可挂在调度关键路径、主特征 SM Active 等可解释、DCGM 计数器跨框架泛化，MAPE 从 Lucid 的 68.72% 降到 31.84%，干扰预测 R² 0.902。抢占用 E_j = R_j·T̂/(1+α·C_p) 贪心选 victim，分子是可回收 GPU-时、分母惩罚反复被抢占。共享靠 RF 准入加在线检测兜底——连续 3 窗口 retention 低于 ρ_tol 就驱逐。论文承认四条局限：干扰模型只在 16 个模型族上训练、testbed 仅 16 GPU、异构卡需重训、未覆盖分布式训练 Gang Scheduling。和 prior art 的本质差异是用单一 QAD 信号把配额保障、干扰合用、运行时预测三者闭环，Tiresias/Lucid/Gavel/HiveD 都只解决其中一两块。
