...
多租户管理的本质是在保障每个租户基本权益的前提下最大化利用率。固定配额强保障但利用率只有 30-40%，ElasticQuota 用 min/max + 抢占回收换弹性，QAD 驱动用连续的"配额健康度"做更精细的回收、利用率能到 80%+。配额之外还要叠加 Namespace、Queue、节点、GPU（MIG/MPS）四层隔离防止租户互相干扰。

**2 分钟版：**

多租户管理要解决的核心矛盾，是在保障每个租户基本权益和最大化集群利用率之间找平衡。配额模型有四种：固定配额（ResourceQuota）强保障但利用率只有 30-40%，闲置资源不能借用；ElasticQuota 给每个租户 min 保障量和 max 上限，闲时可借用、被借方需要时通过抢占回收，但抢占是简单优先级、不考虑沉没成本；QAD 驱动用 QAD（实际获得资源时间/应获得时间）这个连续的"配额健康度"信号，无 max 上限借用、QAD 低于阈值时优先回收，利用率能到 80%+；DRF 则解决多维资源的公平性而非保障性。配额管理之外还要做隔离，从粗到细是 Namespace（ResourceQuota/LimitRange 资源量隔离）、Queue（Volcano/Kueue 弹性配额 + 公平调度 + Gang）、节点（Taint/Affinity 专属）、GPU（MIG 硬件切片做强隔离、MPS 软件共享提利用率）。落到 AI Infra 场景：训练任务对性能波动敏感用 MIG 或独占，推理和数据预处理可用 MPS 共享；设计配额系统的关键是讲清 guarantee 怎么定、借用怎么管、回收怎么触发、抢占代价怎么控，并通过 QAD 看板和利用率趋势持续验证保障是否真的有效。
