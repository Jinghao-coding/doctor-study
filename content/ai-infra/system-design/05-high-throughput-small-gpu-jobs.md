...
这个场景的核心不是“给每个作业找一张 GPU”，而是把 GPU 显存当成主要可切分资源，在异构 GPU、异构 CPU/内存配比和海量短作业到达流下做**在线多维装箱**：先补齐资源画像和调度观测，再做显存主导的分层队列、候选机器快速索引、best-fit/bin packing、短作业 backfill、运行时反馈校准，最后再引入预测和批量调度优化吞吐。
