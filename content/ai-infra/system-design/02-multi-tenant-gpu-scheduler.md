...
GPU 集群调度不是「找有空卡的节点」，而是在多租户公平、gang 语义、拓扑质量、碎片治理和抢占代价之间做持续权衡。我会先分清训练/推理/评测/数据四类 workload 的调度语义，再把 GPU 抽象成型号+显存+拓扑+网络+故障域，然后讲队列、配额、抢占、回填的组合。

**2 分钟版：**

我会按「任务抽象 → 资源抽象 → 调度目标 → 调度策略 → 拓扑感知 → 故障处理 → 观测 → 性能优化」的顺序展开（对应上面的分节）。重点突出三件事：第一，公平性用 QAD 连续保障度而不是二元配额，配合 DRA 弹性借用——闲时把资源借出去、需要时按保障度优先回收，排序用词典序先补最欠缺的租户、同等级短作业优先。第二，gang + 拓扑是 AI 调度区别于普通 K8s 调度的核心，训练任务必须满足 minAvailable 才启动，TP 要同节点 NVLink、PP 相邻 stage 低延迟、DP 可跨节点但要好 RDMA。第三，抢占要 checkpoint-aware，按释放资源量和沉没成本算代价，混部要干扰感知、用 MIG/MPS 隔离并实时监控保护主任务。最后用可观测收束：等待原因、利用率、JCT、QAD、失败率这些指标是判断调度策略好坏的依据。
