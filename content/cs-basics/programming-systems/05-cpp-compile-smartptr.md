...
C++ 从源码到可执行文件分四阶段：预处理做宏替换和头文件展开、编译生成汇编、汇编生成目标文件、链接做符号解析和重定位。配套两个高频点：库分静态库（编译时整体复制进可执行文件）和动态库（运行时加载、多进程共享一份）；智能指针 unique_ptr 独占、shared_ptr 引用计数共享、weak_ptr 解循环引用。多线程死锁则用 gdb attach 后逐线程看堆栈。

**2 分钟版：**

源码到可执行文件走四个阶段：预处理做宏替换、头文件展开、条件编译，编译阶段做语法语义分析、生成中间码并优化出汇编，汇编把指令转成目标文件，链接做符号解析和重定位、把引用绑到实际地址，最终产出可执行文件或库。库有静态库和动态库之分：静态库编译时整体复制进可执行文件，体积大但无运行依赖；动态库运行时加载、多进程共享一份、替换 .so 即可更新，但要求运行环境能找到匹配版本——AI Infra 里 CUDA/cuDNN/NCCL 都是动态库，最常见就是运行时找不到 .so 或版本不匹配，用 ldd 和 LD_LIBRARY_PATH 排查。智能指针方面，unique_ptr 独占、零开销，shared_ptr 引用计数共享，weak_ptr 不增计数、用来打破 shared_ptr 循环引用；自己实现引用计数指针要有原始指针、计数器、拷贝构造加计数、赋值时新增旧减、析构计数归零再释放。最后是排障：C++ 多线程卡死用 gdb attach 进程后 info threads、切线程 bt，典型死锁是多个线程都停在 pthread_mutex_lock 互相等待；Python 侧大模型训练 hang 则用 py-spy dump 看各 rank 卡点。
