...
面向对象三大特性：封装是隐藏实现、只暴露接口，继承是复用和扩展，多态是同名不同行为。C++ 的多态分两类——编译期的重载和模板，运行期的虚函数。运行期多态靠虚函数表实现：含虚函数的类有一张 vtable，对象最前面存 vptr 指向它，所以基类指针调用虚函数能分派到子类实现，也因此基类析构必须 virtual。

**2 分钟版：**

封装、继承、多态是三大特性：封装把数据和操作绑在一起、对外只给接口做信息隐藏；继承让子类复用父类、表达 is-a；多态是同一函数名有多种行为。C++ 里多态分编译期和运行期：重载是同作用域同名不同参，编译期按实参静态决议；重写是子类重定义父类虚函数，运行期动态分派。运行期多态的机制是虚函数表——编译器为每个含虚函数的类生成一张 vtable 存各虚函数地址，每个对象实例最前面放一个 vptr 指向所属类的 vtable，通过基类指针调用时先取 vptr、再按固定偏移找到函数地址间接调用。两个高频追问点：一是基类析构不声明 virtual，用基类指针 delete 子类对象时子类析构不会被调用，造成资源泄漏；二是访问控制和继承方式（public/protected/private）会改变成员在子类中的可见性。这套机制在推理引擎、算子注册这类用基类接口统一调度不同后端实现的场景里非常常见。
