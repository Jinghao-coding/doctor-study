...
TCP 面向连接、提供可靠有序的字节流，靠序列号、ACK、重传、流控和拥塞控制实现，开销高，用在 HTTP/gRPC、数据库、可靠 RPC；UDP 无连接、不保证可靠和有序，但开销低，用在 DNS、实时音视频、QUIC 底层。关键是 TCP 的「可靠」是机制堆出来的，不是天生的，真实系统还要处理 TIME_WAIT 堆积、队头阻塞这些坑。

**2 分钟版：**

TCP 和 UDP 的核心区别是：TCP 面向连接、用序列号+ACK+超时/快速重传保证可靠有序，靠滑动窗口做流控（接收方 rwnd 防打爆对端缓冲）、靠 cwnd 做拥塞控制（发送方防打爆网络，经典慢启动→拥塞避免→快速重传→快速恢复四阶段），代价是连接管理开销；UDP 无连接、不保证可靠，但延迟和开销都低。连接管理上，三次握手本质是双向确认收发能力并同步初始序列号，四次挥手是因为全双工要分别关闭两个方向、被动方的 ACK 和 FIN 通常不能合并，主动方最后停在 TIME_WAIT 一个 2MSL 既保证最后 ACK 可达又让旧报文消亡。一个高频工程坑是短连接高并发会堆积大量 TIME_WAIT 耗尽端口，正解是用长连接/连接池而不是无脑开 tcp_tw_reuse。还有队头阻塞：TCP 字节流有序交付，一个包丢了后续都得等，HTTP/2 在单条 TCP 上多路复用仍逃不掉 TCP 层 HoL，HTTP/3 改用基于 UDP 的 QUIC、让流之间独立才根治。落到 AI Infra，RPC 框架、参数服务器这类可靠通信都跑在 TCP 上，连接池设计和拥塞算法（数据中心用 BBR）直接影响吞吐和尾延迟。
