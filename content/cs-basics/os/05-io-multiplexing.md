...
I/O 多路复用解决的是单线程同时处理海量连接的问题——大部分连接大部分时间空闲，一连接一线程会线程爆炸。它让一个线程用 select/poll/epoll 一次监听大量 fd，只处理就绪的。epoll 是主流：fd 注册一次常驻内核，epoll_wait 只返回就绪 fd，在"连接多、活跃少"的 C10K 场景远胜 select/poll。

**2 分钟版：**

I/O 多路复用要解决的核心矛盾是：一个服务要同时维护成千上万条连接，但大部分连接大部分时间是空闲的，如果一连接一线程会线程爆炸、上下文切换吃光 CPU。多路复用让单线程用一次系统调用监听大量 fd，只在就绪时才去处理。select/poll/epoll 的关键区别在于：select/poll 每次调用都要把全部 fd 集合从用户态拷到内核态、返回后还要 O(n) 线性扫描找就绪的，而且 select 有 FD_SETSIZE 上限；epoll 用 epoll_ctl 把 fd 注册一次常驻内核红黑树，事件就绪由回调挂到就绪链表，epoll_wait 直接返回就绪 fd 列表，复杂度只和活跃连接数相关，所以在海量连接少量活跃时优势最明显，但连接很少或几乎都活跃时三者差不多。还要会讲 LT 和 ET：ET 只在状态变化时通知一次，必须配非阻塞 fd 并循环读到 EAGAIN，否则丢事件、连接假死。落到 AI Infra：推理网关、参数服务器、KV 存储、调度器 watch 本质都是少量线程加 epoll 事件循环（Reactor 模式），io_uring 则进一步把网络和磁盘 I/O 改成异步提交-完成队列，减少系统调用次数，是新一代高吞吐 I/O 方向。
