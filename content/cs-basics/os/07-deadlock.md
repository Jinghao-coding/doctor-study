...
死锁是一组线程互相持有对方需要的资源、谁都进行不下去，必须互斥、持有并等待、不可剥夺、循环等待四条同时满足，破坏任一条即可预防。工程上最实用的是破坏循环等待——给所有锁定义全局顺序、统一顺序加锁，再加锁超时回退兜底。分布式训练里某个 rank 没进入 NCCL 集合通信导致整作业 hang，本质也是这种互相等待。

**2 分钟版：**

死锁是一组进程或线程互相持有对方需要的资源、谁都无法继续推进。它需要四个必要条件同时成立：互斥、持有并等待、不可剥夺、循环等待，破坏任意一个就能预防。处理策略有预防、避免、检测、恢复四类，但银行家算法这种避免方案要预知最大需求、实际很少用；业务系统普遍采用"预防加超时"：给所有锁定义全局顺序、任何线程都按同一顺序加锁，环就不可能形成，再用 try_lock 加超时回退兜底，并尽量减小锁粒度、缩短临界区。排查上要会区分死锁、活锁、饥饿：死锁是 CPU 不忙但卡死、线程栈都停在 lock，用 gdb/pstack 或 Java 的 jstack 直接看；活锁是不停重试都没进展、CPU 很忙，靠随机退避打破；饥饿是某线程长期抢不到、用公平锁或 aging。落到 AI Infra：NCCL all-reduce 要求所有 rank 都到达，某个 rank 异常退出或走了不同分支，其他 rank 会在通信原语上无限等待表现为整作业 hang，用 py-spy dump 看各 rank 卡在哪；多个大作业各占部分 GPU 又都等不齐完整资源会形成资源死锁，要靠 gang scheduling 破解。
